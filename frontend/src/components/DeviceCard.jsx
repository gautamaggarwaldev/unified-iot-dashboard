...
const DeviceCard = ({ device }) => {
  ...
  return (
    <Link to={`/devices/${device.deviceId}`} className="group block">
      <div className="relative bg-white dark:bg-slate-800 rounded-lg border border-slate-200 dark:border-slate-700 overflow-hidden transition-all duration-300 hover:shadow-lg hover:border-teal-500 dark:hover:border-teal-400">
        ...
        <div className="p-4 sm:p-5 pl-5 sm:pl-6">
          ...
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-3 mb-3">
            <div className="flex-1 min-w-0">
              <h3 className="font-semibold text-base sm:text-lg text-slate-900 dark:text-white truncate mb-1">
                {device.deviceId}
              </h3>
          ...
          <div className="flex items-start gap-2 pt-3 border-t border-slate-100 dark:border-slate-700">
            <svg className="w-4 h-4 text-slate-400 dark:text-slate-500 mt-0.5 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
